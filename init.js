...
(function init() {
    checkRedisVersion(function () {
        if (singleModule) {
            global.log("info", logSystem, "Running in single module mode: %s", [
                singleModule,
            ]);
    ...
    global.redisClient.info(function (error, response) {
        ...
        if (!version) {
            global.log(
                "error",
                logSystem,
                "Could not detect redis version - must be super old or broken"
            );
            return;
        } else if (version < 2.6) {
            global.log(
                "error",
                logSystem,
                "You're using redis version %s the minimum required version is 2.6. Follow the damn usage instructions...",
                [versionString]
            );
            return;
        }
...
function spawnPoolWorkers() {
    if (
        !global.config.poolServer ||
        !global.config.poolServer.enabled ||
        !global.config.poolServer.ports ||
        global.config.poolServer.ports.length === 0
    )
        return;
    ...
    if (global.config.poolServer.ports.length === 0) {
        global.log(
            "error",
            logSystem,
            "Pool server enabled but no ports specified"
        );
        return;
    }
    ...
    var numForks = (function () {
        ...
        if (isNaN(global.config.poolServer.clusterForks)) {
            return 1;
        }
        ...
        worker
            .on("exit", function (code, signal) {
                global.log(
                    "error",
                    logSystem,
                    "Pool fork %s died, spawning replacement worker...",
                    [forkId]
                );
                setTimeout(function () {
            ...
            .on("message", function (msg) {
                switch (msg.type) {
                    ...
                    case "shareTrust":
                        Object.keys(cluster.workers).forEach(function (id) {
                            if (
                                cluster.workers[id].type === "pool" &&
                                cluster.workers[id].forkId !== worker.forkId
                            ) {
                                cluster.workers[id].send({
                                    type: "shareTrust",
                                    ip: msg.ip,
                                    address: msg.address,
                                    shareValidated: msg.shareValidated,
                                });
                            }
                        });
                        break;
    ...
    var spawnInterval = setInterval(function () {
        ...
        if (i - 1 === numForks) {
            ...
            global.log("info", logSystem, "Pool spawned on %d thread(s)", [
                numForks,
            ]);
        }
    }, 10);
...
function spawnBlockUnlocker() {
    if (!global.config.blockUnlocker || !global.config.blockUnlocker.enabled)
        return;
