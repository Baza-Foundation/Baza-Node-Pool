...
try {
  ...
  global.log("error", logSystem, "Pool server address is invalid", [
    global.config.poolServer.poolAddress
  ]);
  process.exit(1);
}
...
function runInterval() {
  async.waterfall(
    [
      // Get worker keys
      function(callback) {
        global.redisClient.keys(global.config.coin + ":workers:*", function(
          error,
          result
        ) {
          if (error) {
            global.log(
              "error",
              logSystem,
              "Error trying to get worker balances from redis %j",
              [error]
            );
            callback(true);
            return;
        ...
        global.redisClient.multi(redisCommands).exec(function(error, replies) {
          if (error) {
            global.log(
              "error",
              logSystem,
              "Error with getting balances from redis %j",
              [error]
            );
            callback(true);
            return;
          ...
          for (var i = 0; i < replies.length; i++) {
            ...
            minPayoutLevel[workerId] =
              parseFloat(data[1]) || global.config.payments.minPayment;
            try {
              ...
              if (minerAddress.paymentId.length !== 0) {
                if (
                  minPayoutLevel[workerId] <
                  global.config.payments.minPaymentIdPayment
                ) {
                  minPayoutLevel[workerId] =
                    global.config.payments.minPaymentIdPayment;
                }
              ...
              global.log(
                "error",
                logSystem,
                "Skipping invalid miner payment address %s",
                [workerId]
              );
              continue;
            }
            global.log(
              "info",
              logSystem,
              "Using payout level %d for worker %s (default: %d)",
              [minPayoutLevel[workerId], workerId, defaultPaymentThreshold]
            );
          }
      ...
      function(balances, minPayoutLevel, callback) {
        ...
        if (Object.keys(payments).length === 0) {
          global.log(
            "info",
            logSystem,
            "No workers' balances reached the minimum payment threshold"
          );
          callback(true);
          return;
        }
        ...
        for (worker in payments) {
          ...
          try {
            ...
            global.log(
              "error",
              logSystem,
              "Skipping payment to invalid miner payment address %s",
              [worker]
            );
            continue;
          }
          ...
          if (
            global.config.payments.maxTransactionAmount &&
            amount + commandAmount > global.config.payments.maxTransactionAmount
          ) {
            amount =
              global.config.payments.maxTransactionAmount - commandAmount;
          }
          ...
          transferCommands[commandIndex].redis.push([
            "hincrby",
            global.config.coin + ":workers:" + worker,
            "balance",
            -amount
          ]);
          transferCommands[commandIndex].redis.push([
          ...
          if (
            addresses >= global.config.payments.maxAddresses ||
            (global.config.payments.maxTransactionAmount &&
              commandAmount >= global.config.payments.maxTransactionAmount) ||
        ...
        async.filter(
          transferCommands,
          function(transferCmd, cback) {
            apiInterfaces.rpcWallet(
              "sendTransaction",
              transferCmd.rpc,
              function(error, result) {
                if (error) {
                  global.log(
                    "error",
                    logSystem,
                    "Error with sendTransaction RPC request to wallet daemon %j",
                    [error]
                  );
                  global.log(
                    "error",
                    logSystem,
                    "Payments failed to send to %j",
                    transferCmd.rpc.transfers
                  );
                  cback(false);
                  return;
                }
                ...
                transferCmd.redis.push([
                  "zadd",
                  global.config.coin + ":payments:all",
                  now,
                  [
                    txHash,
                    transferCmd.amount,
                ...
                for (var i = 0; i < transferCmd.rpc.transfers.length; i++) {
                  ...
                  transferCmd.redis.push([
                    "zadd",
                    global.config.coin + ":payments:" + destination.address,
                    now,
                    [txHash, destination.amount, transferCmd.rpc.fee].join(":")
                  ]);
                }
                ...
                global.log(
                  "info",
                  logSystem,
                  "Payments sent via wallet daemon %j",
                  [result]
                );
                global.redisClient
                  .multi(transferCmd.redis)
                  .exec(function(error, replies) {
                    if (error) {
                      global.log(
                        "error",
                        logSystem,
                        "Super critical error! Payments sent yet failing to update balance in redis, double payouts likely to happen %j",
                        [error]
                      );
                      global.log(
                        "error",
                        logSystem,
                        "Double payments likely to be sent to %j",
                        transferCmd.rpc.transfers
                      );
                      cback(false);
                      return;
          ...
          function(succeeded) {
            ...
            global.log(
              "info",
              logSystem,
              "Payments splintered and %d successfully sent, %d failed",
