...
var bannedIPs = {};
var perIPStats = {};
// NOTE: After seeing certain socket flood attack and malformed message related to windows RDP protocol exploit
// source: https://github.com/olipo186/Git-Auto-Deploy/issues/199
// This was implemented, there is some room for improvement, 1. Now, if it sees any malformed message or socket flood
// attack, it will ban immediately this should be changed and banned only if multiple times 2. Maybe in future we should add
// a mechanism to remove ban if someone requests
var permanentBannedIPs = [];

var banningEnabled =
    global.config.poolServer.banning &&
    global.config.poolServer.banning.enabled;

var addressBase58Prefix = cnUtil.address_decode(
    new Buffer(global.config.poolServer.poolAddress)
);

try {
    const poolAddress = turtleUtil.decodeAddress(
        global.config.poolServer.poolAddress,
        addressBase58Prefix
    );
    if (!poolAddress) throw new Error("Could not decode address");
} catch (e) {
    log("error", logSystem, "Pool server address is invalid", [
        global.config.poolServer.poolAddress,
    ]);
    process.exit(1);
}
...
setInterval(function () {
    ...
    if (banningEnabled) {
        for (var ip in bannedIPs) {
            log("info", logSystem, "Banned IP %s", [ip]);
...
process.on("message", function (message) {
    switch (message.type) {
        case "banIP":
            ...
            break;
        case "permanentBanIP":
            let ipInfo = permanentBannedIPs.find((x) => x.ip === message.ip);
            if (ipInfo) {
                ipInfo.count += 1;
            } else {
                ipInfo = {
            ...
            permanentBannedIPs.push(ipInfo);
            break;
    }
});
...
function jobRefresh(loop, callback) {
    ...
    getBlockTemplate(function (error, result) {
        ...
        if (error) {
            log("error", logSystem, "Error polling getblocktemplate %j", [
                error,
            ]);
            callback(false);
            return;
...
var VarDiff = (function () {
    var variance =
        (global.config.poolServer.varDiff.variancePercent / 100) *
        global.config.poolServer.varDiff.targetTime;
    return {
        variance: variance,
        bufferSize:
            (global.config.poolServer.varDiff.retargetTime /
                global.config.poolServer.varDiff.targetTime) *
            4,
...
Miner.prototype = {
    ...
    setNewDiff: function (newDiff) {
        ...
        log("info", logSystem, "Retargetting difficulty %d to %d for %s", [
            this.difficulty,
            newDiff,
            this.login,
        ]);
    ...
    checkBan: function (validShare) {
        ...
        if (
            ...
            global.config.poolServer.banning.checkThreshold
        ) {
            if (
                stats.invalidShares /
                    (stats.invalidShares + stats.validShares) >=
                global.config.poolServer.banning.invalidPercent / 100
            ) {
...
function recordShareData(
    ...
    if (global.config.poolServer.slushMining.enabled) {
        if (
            lastChecked +
                global.config.poolServer.slushMining.lastBlockCheckRate <=
                dateNowSeconds ||
            lastChecked === 0
        ) {
            global.redisClient.hget(
                global.config.coin + ":stats",
                "lastBlockFound",
        ...
        job.score =
            ...
            Math.pow(
                ...
                (scoreTime - dateNowSeconds) /
                    global.config.poolServer.slushMining.weight
            ); // Score Calculation
        log(
            "info",
            logSystem,
    ...
    var redisCommands = [
        [
            "hincrby",
            global.config.coin + ":shares:roundCurrent",
            miner.login,
            job.score,
        ],
        [
            "zadd",
            global.config.coin + ":hashrate",
            dateNowSeconds,
            [
                job.difficulty,
        ...
        [
            "hincrby",
            global.config.coin + ":workers:" + miner.login,
            "hashes",
            job.difficulty,
        ],
        [
    ...
    if (blockCandidate) {
        ...
        redisCommands.push([
            "rename",
            global.config.coin + ":shares:roundCurrent",
            global.config.coin + ":shares:round" + job.height,
        ]);
        redisCommands.push([
            "hgetall",
            global.config.coin + ":shares:round" + job.height,
    ...
    global.redisClient.multi(redisCommands).exec(function (err, replies) {
        ...
        if (blockCandidate) {
            ...
            global.redisClient.zadd(
                global.config.coin + ":blocks:candidates",
                job.height,
                [
                    hashHex,
                    (Date.now() / 1000) | 0,
    ...
    log("info", logSystem, "Accepted %s share at difficulty %d/%d from %s@%s", [
        shareType,
        job.difficulty,
        shareDiff,
        miner.login,
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    var shareBuffer = cnUtil.construct_block_blob(
        template,
    ...
    log(
        "info",
        logSystem,
        "Processing block v%d.%d share for %s@%s at height %d",
        [shareBuffer[0], shareBuffer[1], miner.login, miner.ip, job.height]
    );
    ...
    if (
        shareTrust.enabled &&
        shareTrust.checkTrust(miner.ip, miner.login, job.difficulty)
    ) {
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) {
        apiInterfaces.rpcDaemon(
            ...
            function (error, result) {
                if (error) {
                    ...
                    var blockFastHash = cnUtil
                        .get_block_id(shareBuffer)
                        .toString("hex");
                    log(
                        "info",
                        logSystem,
        ...
        log(
            ...
            [hashDiff.toString(), miner.login, miner.ip]
        );
        if (shareTrust.enabled) {
            shareTrust.setTrust(miner.ip, miner.login, false);
        }
       ...
       rules of the protocol and needs banned */
        ...
        if (typeof miner === "undefined") {
            log(
                "error",
                logSystem,
                "Miner for %s not found in our connected miner list",
                [ip]
            );
            return;
        }
    ...
    switch (method) {
        case "login":
            ...
            if (loginParts.length === 2) {
                ...
                log("info", logSystem, "Miner %s uses worker name: %s", [
                    loginParts[0],
                    workerName,
                ]);
            }
         ...
         payment id, etc */
            ...
            if (loginParts.length >= 2) {
                for (var i = 1; i < loginParts.length; i++) {
                    ...
                    if (
                        toNumber(part) &&
                        global.config.poolServer.fixedDiff.enabled
                    ) {
                        /* This is a number so it's probably a fixed diff */
                        noRetarget = true;
                        difficulty = toNumber(part);
                        log("info", logSystem, "Miner difficulty fixed to %s", [
                            difficulty,
                        ]);
                    } else if (
                        isPaymentId(part) &&
                        global.config.payments.allowPaymentId
                    ) {
           ...
           prefix of the pool server itself */
                minerAddress = turtleUtil.decodeAddress(
                    login,
                    addressBase58Prefix
                );
             ...
             found in the integrated address */
                ...
                if (minerAddress.paymentId.length !== 0) {
                    log("info", logSystem, "Miner submitted payment ID %s", [
         ...
         they can not do that here */
            if (
                !global.config.payments.allowPaymentId &&
                minerAddress.paymentId.length !== 0
            ) {
                sendReply(
                    "mining to an address with a payment id is not supported here"
                );
                return;
            }

            /* Rebuild the miner address for the login (due to payment ids) */
            login = turtleUtil.encodeAddress(
        ...
        case "submit":
            ...
            if (!noncePattern.test(params.nonce)) {
                ...
                perIPStats[miner.ip].invalidShares += Math.floor(
                    (global.config.poolServer.banning.checkThreshold / 4) *
                        (global.config.poolServer.banning.invalidPercent /
                            100) -
                        1
                );
            ...
            var shareAccepted = processShare(
                miner,
                job,
                blockTemplate,
                params.nonce,
...
function startPoolServerTcp(callback) {
    async.each(
        global.config.poolServer.ports,
        function (portData, cback) {
            ...
            net.createServer(function (socket) {
                ...
                socket
                    .on("data", function (d) {
                        const ipInfo = permanentBannedIPs.find(
                            (x) => x.ip === socket.remoteAddress
                        );
                        if (ipInfo && ipInfo.count >= 3) {
                            log(
                                "warn",
                                logSystem,
                                "IP is banned permanently: %s",
                                [socket.remoteAddress]
                            );
                            socket.destroy();
                        } else {
                            ...
                            log(
                                "info",
                                logSystem,
                                "Data Buffer size from IP %s: %s",
                                [
                                    socket.remoteAddress,
                                    Buffer.byteLength(dataBuffer, "utf8"),
                                ]
                            );
                            if (Buffer.byteLength(dataBuffer, "utf8") > 71680) {
                                // 70KB
                                dataBuffer = null;
                                log(
                                    "warn",
                                    logSystem,
                                    "Socket flooding detected and prevented from the IP and flagged to ban %s",
                                    [socket.remoteAddress]
                                );
                                process.send({
                                    type: "permanentBanIP",
                                ...
                                for (var i = 0; i < messages.length; i++) {
                                    ...
                                    try {
                                        ...
                                        if (message.indexOf("GET /") === 0) {
                                            if (
                                                message.indexOf("HTTP/1.1") !==
                                                -1
                                            ) {
                                                socket.end(
                                                    "HTTP/1.1" + httpResponse
                                                );
                                                break;
                                            } else if (
                                                message.indexOf("HTTP/1.0") !==
                                                -1
                                            ) {
                                                socket.end(
                                                    "HTTP/1.0" + httpResponse
                                                );
                                                break;
                                            }
                                        }
                                        ...
                                        log(
                                            "warn",
                                            logSystem,
                                            "Malformed message from the IP and flagged to ban %s: %s",
