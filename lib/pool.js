...
process.on("message", function (message) {
    switch (message.type) {
        case "banIP":
            ...
            break;
        case "permanentBanIP":
            log("warn", logSystem, "Got Ip to ban: %s", [message.ip]);
            permanentBannedIPs.push(message.ip);
            break;
    }
});
    ...
    async.each(
        ...
        function (portData, cback) {
            ...
            net.createServer(function (socket) {
                ...
                socket
                    .on("data", function (d) {
                        log("warn", logSystem, "permanentBannedIps: %s", [
                            permanentBannedIPs,
                        ]);
                        log("warn", logSystem, "currIP: %s", [
                            socket.remoteAddress,
                        ]);
                        if (
                            permanentBannedIPs.indexOf(socket.remoteAddress) ===
                            -1
                        ) {
                            dataBuffer += d;
                            if (Buffer.byteLength(dataBuffer, "utf8") > 10240) {
                                ...
                                log(
                                    "warn",
                                    logSystem,
                                    "Socket flooding detected and prevented from %s and banned the IP",
                                    [socket.remoteAddress]
                                );
                                process.send({
                                    type: "permanentBanIP",
                                ...
                                for (var i = 0; i < messages.length; i++) {
                                    ...
                                    try {
                                        ...
                                        log(
                                            "warn",
                                            logSystem,
                                            "Malformed message from %s: %s and banned the IP",
